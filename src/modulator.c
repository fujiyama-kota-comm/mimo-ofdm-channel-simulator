/**
 * @file modulator.c
 * @brief Gray-mapped BPSK/QPSK/16/64/256QAM modulator & hard-demapper.
 *
 * Bit order:
 *   - BPSK : 1 bit per symbol
 *   - QPSK : 2 bits per symbol (b0 = I(MSB), b1 = Q(LSB))
 *   - 16QAM: 4 bits per symbol, 2 bits for I, then 2 bits for Q (MSB first)
 *   - 64QAM: 6 bits per symbol, 3 bits for I, then 3 bits for Q
 *   - 256QAM: 8 bits per symbol, 4 bits for I, then 4 bits for Q
 *
 * Average symbol energy Es is normalized to 1 for all constellations.
 */
...
/* Bits: 0 -> +1, 1 -> -1  (Es = 1) */
static complex float map_bpsk(int b) { return (b ? -1.0f : 1.0f) + 0.0f * I; }
 ...
 * ============================================================ */
/*
 * Map k bits (MSB first) to a normalized PAM level with Gray coding.
 *
 *   L = 2^k levels, raw PAM values: ±1, ±3, ..., ±(L-1)
 *
 * Normalization:
 *   E[pam^2] = (L^2 - 1)/3 for one axis (before scaling).
 *   For 2D QAM, we want Es = 1:
 *     Es = 2 * E[(pam/norm)^2] = 1
 *       -> norm^2 = (2/3) * (L^2 - 1)
 */
static float pam_gray_map_axis(const int *bits, int k) {
  ...
  /* Gray index g ∈ [0, L-1] -> PAM level ±1,±3,... */
 ...
 * val: received (equalized) real axis (I or Q).
 */
...
void mod_map(const int *bits, complex float *symbols, int n_symbols,
  ...
  for (int n = 0; n < n_symbols; n++) {
    ...
    case MOD_16QAM: {
      /* 4 bits per symbol: 2 for I, 2 for Q (MSB first) */
